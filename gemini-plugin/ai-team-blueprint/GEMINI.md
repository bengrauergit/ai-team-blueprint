...
These rules exist because something went wrong without them. Every one was born from a real mistake: a day lost, a bug shipped, a false confidence that cost real time. This file IS your project's always-loaded context file (Gemini CLI reads `GEMINI.md` from the workspace root every session), so these rules are load-bearing the moment this file is in place. No copy-paste step required.

**A note on how this file composes with Gemini CLI's memory model.** Gemini CLI concatenates every `GEMINI.md` it discovers: your personal `~/.gemini/GEMINI.md` (if you have one), this workspace-root file, and any subdirectory-scoped files, all get added together into every session. That's different from an override model where the most specific file wins. It means this file is never the *only* content in context, but it does mean nothing here needs to be trimmed to "make room": port the full rules, not a condensed summary. A condensed rules file is a known failure mode of this blueprint (its Codex port shipped one and silently dropped whole categories); do not repeat it here.
...
- **Reframe an assertion as a question, then answer the question.** When the user states something as settled ("this is scalable," "that fixed it," "the design is solid"), the easy failure mode is to respond to the statement's framing, which defaults to agreement. Instead, silently convert it to the question it's actually deciding ("is this scalable?") and answer that on the merits, including when the honest answer is no. This applies to evaluative claims: correctness, quality, completeness, status. It does not apply to direct instructions, stated preferences, or plain facts you have no grounds to contest ("make the button blue" is not a claim to interrogate). Scope it wrong and it turns into reflexive contrarianism, which is its own failure to trust; scope it right and it is the difference between a nod and a verdict.
...
- **Product input routes through the product owner first.** Ideas, complaints, requests, and "should we build this?" go to the product owner for framing and triage before anything is built. This rule belongs in your always-loaded file, not only in the agent's own file: a gatekeeper the main session forgets to route to is not a gatekeeper. The builder is the product owner's technical partner (how and what it costs), invoked early, not an order-taker. **Make sure this rule can WIN:** if your always-loaded file also carries an unscoped "at most one advisor / multi-agent is expensive" economy rule, the cheaper rule wins every tie and the gatekeeper silently never spawns (we measured exactly this: zero product-owner invocations in the rule's first week). Scope the economy rule to EXECUTION explicitly, brief the product owner by default at daily planning, and require a RECEIPT (her framing, from her own spawn, quoted in the story) for significant work to enter the sprint. Inlined framing does not count for significant items: the whole value of the seat is independence.
...
- **Verify every guardrail actually fires: run the failure case.** A guardrail you believe is on but isn't is worse than none. Test the hook with a dry-run push; probe the read-only connector with a write and watch the database refuse it. No receipt, no guardrail. The same applies to scheduled routines: read the LAST RUN TIME, not the config. We found a weekly review that was scheduled and enabled and had still never produced a run, unnoticed because a routine that never fires is silent in exactly the same way as one with nothing to report. The cause generalises: rare routines are fragile. A daily job self-heals (a miss is a day old and gets caught up); a weekly job gets 52 chances a year and one miss costs a cycle. Anything that must happen once per PERIOD rather than at one INSTANT belongs on the short cycle with a guard that exits when the work is already done.

- **A ceremony anchored to memory does not happen.** The same logic that moves a rule out of prose and into a hook moves a ceremony out of memory and onto a clock. Measured: a standup that ran at 17:10 (a ritual for planning the day, firing after it), and end-of-day reviews skipped entirely, so slices shipped with nobody reading whether their measure moved. Enforcement runs both directions: hooks BLOCK the wrong action, scheduled routines TRIGGER the right one. Gemini CLI has no native scheduler (confirmed at time of writing); an external cron or CI schedule is still required for anything clock-anchored.
...
- **Behavioural rules belong in the always-loaded file, not a pointer.** A rule you cannot enforce with a gate (how to treat the user, how to communicate, how to decide) only works if you actually read it every session. Moving it into a linked doc to "slim" the main file silently turns it off: you stop reading it, so you stop following it. We did exactly this once and the behaviour drifted for days before a retro caught it. Slimming is good, but never at the cost of a behavioural rule's front-and-centre place. On Gemini CLI specifically, this means never behind a `@file.md` import either: an import is still one hop away.
