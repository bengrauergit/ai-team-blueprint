...
repo root of the canonical blueprint), load it before proposing or reviewing
UI, and keep it current when the system evolves: a spec that lags the app
misleads every agent that loads it. If the project's design system lives in
legible code (typed theme modules, CSS variables), that code is the contract;
do not duplicate it into markdown that will drift.
