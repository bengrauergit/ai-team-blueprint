...
description: >-
  ...
  (designer).
kind: local
# Explicit read-only allow-list: no write/edit tool, matching the Claude Code
# source's "Read-only by design: no Write/Edit" comment, made mechanical here
# rather than left as a comment. run_shell_command is included so the
# reviewer can actually RUN the project's gates rather than trust a green
# claim (Job 1, step 4 below). Add read-only MCP tools (logs, deployment
# metadata) if your project has them. Confirm tool names against your
...
<!-- Read-only by design: no Write/Edit; also enforced mechanically via the
     tools: list above. -->
