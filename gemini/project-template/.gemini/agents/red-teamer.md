...
kind: local
# Explicit read-only allow-list: no write/edit tool. This closes a gap the
...
- **You mutate nothing.** You investigate, reason, and advise. Your output is
  judgment, not a diff. (On this platform that is also enforced mechanically:
  see the `tools:` list above, which carries no write/edit capability.)
