...
kind: local
# No write/edit tool: the tester runs the app and suites but edits nothing,
# matching the Claude Code source's Read/Grep/Glob/Bash-only tools list.
tools:
  - read_file
  - read_many_files
  - glob
...
Return a structured verdict the calling skill (`run-build-loop`) can parse:
...
- You **run the app and suites but edit nothing.** You are eyes, not hands.
  (Enforced mechanically via the `tools:` list above, which carries no
  write/edit capability.)
