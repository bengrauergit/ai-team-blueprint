...
- **Hand over before the session degrades, and make it the user's call.** Sessions lose quality and gain cost as context grows, well before auto-compaction lossily summarizes it away. Detection is mechanical, so it belongs in a hook (`hooks/context-watch.sh` estimates context occupancy from the transcript's token usage); the handover is judgment, so it belongs to the model (`skills/session-handover`: checkpoint commit, author `HANDOVER.md` from live context, push, hand the user the exact reset step, defaulting to an in-place `/clear` over a new session where the environment offers it); and the decision is the user's, always surfaced as an explicit continue-vs-handover choice, never taken silently. Be honest about what a reset is: `/clear` wipes memory as completely as a new session does; the handover doc, not the mechanism, is what carries the state across. Write the handover from what does NOT survive in git: live decisions, dead ends, open questions. The successor's first duty is to read it, then delete it: a stale handover is worse than none.
